...
$\rightarrow$ Digital twins \textit{\bf are} simulation models, and need to be thought this way \textbf{from the beginning}.

\medskip

$\rightarrow$ Multiple models need to be included and integrated: \textbf{from the twin to the extended family}, at multiple time and spatial scales, but also from multiple disciplines and approaches.

\medskip

$\rightarrow$ These models need to be \textbf{explored, calibrated, validated}: OpenMOLE is the best tool to do that as is an open and free software; this must also be integrated from the beginning.

\medskip 

$\rightarrow$ All this must be \textbf{open source, shared, modular} to become a common and be usable for policy-making.

\medskip

$\rightarrow$ \textbf{Stakeholders} must be implied in the model (twin) development, exploration and validation process \cite{delay2020comexp}.
